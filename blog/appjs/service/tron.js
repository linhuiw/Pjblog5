...
var JSON, Library, module, require, exports, define, contrast, resove, include, Class, LoadCssFile, LoadJsFile, LoadJscript;
...
(function(){
	...
	LoadJscript = function(callback, params){
		Helper.log('<script language="javascript" type="text/javascript">' + ('(' + callback.toString() + ')(' + JSON.stringify(params) + ');') + '</script>\n');
	};
	...
	LoadCssFile = function(urls){
		...
		for ( var i = 0 ; i < urls.length ; i++ ){
			if ( urls[i] && urls[i].length > 0 ){
				Helper.log('<link rel="stylesheet" type="text/css" href="' + urls[i] + '">\n');
			}
		}
	};
	...
	LoadJsFile = function(urls){
		...
		for ( var i = 0 ; i < urls.length ; i++ ){
			if ( urls[i] && urls[i].length > 0 ){
				Helper.log('<script language="javascript" type="text/javascript" src="' + urls[i] + '"></script>\n');
			}
		}
	};
	...
	factory.extend('readFile', function(){
		var selector = this.resolve(this.__filename);

		if ( helper.modules[selector] === undefined ){
			var content = helper.loader(selector), percent = '%';
	...
	factory.extend('actuate', function(){
		var params = this.readFile();

		if ( params.merger === false ){
			if ( params.content.length > 0 ){
				var wrapper = ['return function (exports, require, module, include, __filename, __dirname, contrast, resolve) { ', params.content, '};'].join("\n"),
					__module = (new Function(wrapper))();
					
				__module( 
					this.exports, 
					helper.proxy(this.require, this), 
					this, 
					helper.proxy(this.include, this), 
					params.selector, 
					this.__dirname, 
					helper.proxy(this.contrast, this), 
					helper.proxy(this.resolve, this)
				);
				
				helper.modules[params.selector] = this.exports;
			}else{
				helper.modules[params.selector] = {};
			}
		}
		
		return helper.modules[params.selector];
	});
	
	factory.extend('require', function( file ){
		var fc = new factory(this.resolve(file));
		return fc.actuate();
	});
